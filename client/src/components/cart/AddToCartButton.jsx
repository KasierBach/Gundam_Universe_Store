...
const AddToCartButton = ({ productId, quantity = 1, disabled = false, className = "" }) => {
  ...
  return (
    <motion.button
      ...
      className={`relative overflow-hidden flex items-center justify-center gap-2 py-2 px-4 rounded font-bold uppercase tracking-widest text-xs transition-all duration-300 ${
        (isAdded || isInCart)
    ...
    >
      <AnimatePresence mode="wait">
        {(isAdded || isInCart) ? (
          ...
          >
            <FiCheck size={16} />
            <span>{isAdded ? "DEPLOYED" : "IN CART"}</span>
          </motion.div>
        ) : (
          <motion.div
            key="idle"
          ...
          >
            ...
            <span>{loading ? "LINKING..." : "ADD UNIT"}</span>
          </motion.div>
        )}
      </AnimatePresence>
