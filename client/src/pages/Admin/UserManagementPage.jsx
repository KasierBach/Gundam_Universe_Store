...
const UserManagementPage = () => {
  ...
  const copy = normalizeLocaleCopy(locale === 'vi'
    ? {
      ...
      subtitle: 'Theo dõi tài khoản và vai trò hệ thống',
      headers: ['Người dùng', 'Vai trò', 'Trạng thái', 'Ngày tham gia'],
      loading: 'Đang đồng bộ danh sách người dùng...',
      active: 'Đang hoạt động',
      disabled: 'Đã khóa',
    }
    : {
      title: 'Pilot Authorization',
      subtitle: 'Monitor accounts and roles',
      headers: ['Pilot', 'Role', 'Status', 'Joined'],
      loading: 'Syncing pilot registry...',
      active: 'Active',
      disabled: 'Disabled',
    })
        ...
        {loading ? (
          ...
          <div key={user._id} className="grid grid-cols-[2fr_1fr_1fr_1fr] gap-4 px-6 py-5 border-b border-white/5 items-center">
            ...
            <div className={`inline-flex items-center gap-2 text-xs uppercase font-orbitron ${user.isActive ? 'text-gundam-emerald' : 'text-gundam-red'}`}>
              <UserCheck size={14} />
              <span>{user.isActive ? copy.active : copy.disabled}</span>
            </div>
