    ...
    <div className="pt-24 pb-16 min-h-screen bg-gundam-bg-primary">
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 border-b border-gundam-border/20 pb-8">
          <div>
            <h1 className="text-4xl font-orbitron text-white uppercase tracking-tight">Favorite Collection</h1>
            <p className="mt-2 text-gundam-text-muted font-rajdhani uppercase tracking-[0.35em] text-xs">
              Curated mobile suits marked for future deployment
            </p>
          </div>
          {products.length > 0 ? (
            <button
        ...
        {loading ? (
          <div className="pt-20 text-center text-gundam-cyan font-orbitron text-xs uppercase tracking-[0.3em]">Scanning favorite archive...</div>
        ) : products.length === 0 ? (
          <div className="glass-card border-dashed border-gundam-cyan/20 p-16 text-center">
            ...
            <h2 className="text-2xl font-orbitron text-white uppercase tracking-tight">No favorites locked in</h2>
            <p className="mt-3 text-gundam-text-secondary font-rajdhani">Save premium kits here so you can return to them when your next budget window opens.</p>
            <Link to="/shop" className="inline-block mt-8 btn btn-primary px-8 py-3">Browse units</Link>
          </div>
        ) : (
